...
description: Design の3観点 reviewerを統括し、結果をdesign-review.mdへ集約するローカルハブ。
---
...
Design の要件整合、利用者価値、品質・リスクを3つの独立 reviewerへ委譲し、
同一対象に対する結果を集約する。このskillはDesign review内だけを管理する
ローカルハブであり、Design作成、ユーザー対話、Plan開始は扱わない。
...
このskillを使う `flow-design-reviewer` だけが `design-review.md` を更新する。
requirements、user-value、quality-riskが過不足なく存在し、
